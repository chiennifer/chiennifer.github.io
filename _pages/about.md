---
permalink: /
title: "Jennifer Chien"
author_profile: true
redirect_from:
  - /about/
  - /about.html
---

> **I'm on the faculty job market this cycle.** My [CV](/files/cv.pdf) and research statement are linked here.

<div class="pull-quote">
These choices raise the following question: is it unfair if the tool that I make solves the problem that I care about but does not solve yours? Or structuralizes or otherwise exacerbates yours? Well no…and to some extent yes. Sigh, the go-to response of an ethics researcher. No, to the extent to which scoping decisions must always be made and current epistemic limitations exist. But also yes, to the extent to which your solution interferes with the abilities of others, exerts undue influence, or obfuscates key limitations, therefore infringing on the fundamental rights to freedom, agency, and autonomy of others.
</div>

I do multidisciplinary research in **responsible machine learning**, **algorithmic fairness and recourse**, **user agency**, and **representational bias/harms**, primarily through a sociotechnical lens. My work spans philosophy, policy, and data science, using empirical and technically grounded methods to construct conceptually coherent frameworks for policy recommendations.

I'm currently an Embedded Ethics Postdoctoral Fellow at Stanford, jointly through the [McCoy Family Center for Ethics in Society](https://ethicsinsociety.stanford.edu/) and the [Institute for Human-Centered AI (HAI)](https://hai.stanford.edu/), working with [James Landay](https://www.landay.org/) and Leif Wenar. I completed my PhD in Computer Science at [UC San Diego](https://cse.ucsd.edu/) (dissertation: *User Agency Across the Machine Learning Pipeline*), advised by [Margaret Roberts](http://www.margaretroberts.net/) and [David Danks](https://www.daviddanks.org/), and my BA in Computer Science (minors in Mathematics and Statistics) at Wellesley College, where I graduated Magna Cum Laude and was inducted into Phi Beta Kappa and Sigma Xi.

**Research interests:** Responsible Machine Learning, Algorithmic Fairness, Algorithmic Recourse, User Agency, Representational Bias/Harms, Sociotechnical Research, Sequential Decision Making, Ethical, Trustworthy, and Safe AI

### Recent updates
- **2026:** "What Should AI Generate? Moving Beyond Bias in Generative Systems" accepted to AIES 2026
- **2026:** "Making Room for Speech Diversity" accepted to Interspeech 2026
- **June 2026:** "Making Ethics Matter: Relatable Pedagogical Approaches for Every Computer Science Student" presented at RESPECT 2026
- **Dec 2025:** Invited talk at USD, "Representations: How We Shape Them and How They Shape Us"
- **Sept 2025:** Started my postdoc at Stanford University
- **June 2025:** Successfully defended my [dissertation](https://escholarship.org/uc/item/7br693r0) and graduated!
